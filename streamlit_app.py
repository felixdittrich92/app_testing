...
def __get_text_from_image_ocrmypdf(image):
  with TemporaryDirectory() as t:
    pdfa = str(t) + '/1.pdfa'
    txt = str(t) + '/1.txt'
    ocrmypdf.ocr(input_file=image , output_file=pdfa, language='deu', optimize=3, redo_ocr=True, sidecar=txt, image_dpi=300, rotate_pages=True, threshold=True, remove_background=True, progress_bar=True)
    file = open(txt, 'r')
    text = file.read()
